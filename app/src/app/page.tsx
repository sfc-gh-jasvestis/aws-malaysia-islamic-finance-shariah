...
export default function HomePage() {
  ...
  const askAiTab = (
    <div className="h-[600px]">
      <AskAI title="Ask AI" sampleQuestions={['How many securities passed latest Shariah screening?', 'Show Islamic vs conventional AUM growth', 'What SAC resolutions are pending?']} onSubmit={async (question, mode) => ({ answer: `[Demo Mode] Response to: "${question}" Connect to Snowflake for live data.`, sql: mode === 'sql' ? 'SELECT * FROM CURATED.SUMMARY LIMIT 10;' : undefined })} />
    </div>
  );
